...
function fncSelectAuthorList(){
    ...
    document.listForm.submit();
//    location.href = "<c:url value='/cms/sec/ram/InfoAuthorList.do'/>";
}

function fncSelectAuthorRole(roleCode) {
    document.listForm.roleCode.value = roleCode;
    document.listForm.action = "<c:url value='/cms/sec/ram/InfoRole.do'/>";
    document.listForm.submit();
}
...
function fncAddAuthorRoleInsert() {
	if(fncManageChecked()) {
	    if(confirm("<spring:message code="comCopSecRam.authorRoleList.validate.confirm.regist" />")) {//등록하시겠습니까?
            document.listForm.action = "<c:url value='/cms/sec/ram/InfoAuthorRoleInsert.do'/>";
            document.listForm.submit();
	    }
	} else return;
}
...
<c:set var="pageTitle">권한롤관리</c:set>
...
<form:form name="listForm" action="${pageContext.request.contextPath}/cms/sec/ram/InfoAuthorRoleList.do" method="post">
	<!-- 검색영역 -->
	<div class="white-box" title="<spring:message code="common.searchCondition.msg" />">
				<input type="text" value="권한코드&nbsp;&nbsp;:" size="1" class="w100" onkeypress="press();" readonly="readonly" /><!-- 부서코드 -->
			<!-- 검색키워드 및 조회버튼 -->
				...
				<input type="button" class="button" value="<spring:message code="button.list" />" onClick="fncSelectAuthorList()" title="<spring:message code="button.list" /> <spring:message code="input.button" />" />
				<input type="button" class="button main" value="<spring:message code="button.create" />" onClick="fncAddAuthorRoleInsert()" title="<spring:message code="button.create" /> <spring:message code="input.button" />" />
	</div>
	...
	<table class="basic" summary="권한롤관리 리스트" />
	<thead>
	<tr>
	...
	<tr>
		...
		<!--
		<div class="btn">
			<span class="btn_s"><a href="<c:url value='/cms/sec/ram/InfoAuthorList.do'/>"  title="<spring:message code="button.list" /> <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
		...
		<input type="hidden" name="pageIndex" value="<c:out value='${infoAuthorRoleManageVO.pageIndex}'/>"/>
		<input type="hidden" name="authorCode" value="<c:out value="${infoAuthorRoleManageVO.searchKeyword}"/>"/>
		<input type="hidden" name="searchCondition">
	<input name="menuTargetNo" id="menuTargetNo" type="hidden" value="${infoAuthorRoleManageVO.menuTargetNo}">
	</form:form>
</div><!-- end div board -->
