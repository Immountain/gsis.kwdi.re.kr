...
function fn_info_select_linkPage(pageNo){
	...
	document.searchVO.action = "<c:url value='/cms/info/popup/popupManageList.do'/>";
...
function fn_pagedetail(popupId) {
	...
	document.searchVO.popupId.value = popupId;
  	document.searchVO.action = "<c:url value='/cms/info/popup/UpdatePopupManageView.do'/>";
  	document.searchVO.submit();
}
...
<div class="sub subView">
	...
	<form name="searchVO" action="<c:url value='/cms/info/popup/popupManageList.do'/>" method="post" onSubmit="fn_info_search_page(); return false;">
		<!-- 검색영역 -->
		<!-- 검색조건선택 -->
		<div class="white-box">
			<div class="rows">
				...
				<button type="button" class="button main" onclick="location.href='<c:url value='/cms/info/popup/RegistPopupManageView.do?menuTargetNo=${menuInfo.menuTargetNo}' />'"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"  ><spring:message code="button.create" /></button>
			</div>
		</div>
			...
			<table class="basic" summary="<spring:message code="common.summary.list" arguments="${pageTitle}" />">
				<thead>
					<tr>
						...
						POPUP_NM is '파업명'
						POPUP_TXT is '파업내용'
						POPUP_IMAGE is '파업 이미지'
						POPUP_WIDTH_LC is '파업가로위치'
						POPUP_WIDTH_SIZE is '파업가로사이즈'
						NTCE_BGNDE is '게시시작일'
						NTCE_ENDDE is '게시종료일'
						STOPVEW_SETUP_AT is '그만보기설정여부'
						NTCE_AT is '게시여부'
						POPUP_VRTICL_LC is '팝업세로위치'
						POPUP_VRTICL_SIZE is '팝업세로 사이즈'
						STYLE_CLASS is '팝업스타일'
						LINK_TYPE is '링크타입'
						LINK_URL is '링크URL'
						TEMP1 is '임시필드1'
						TEMP2 is '임시필드2'
				...
				<tbody>
					...
					<c:forEach items="${list}" var="item" varStatus="status">
						<tr>
							...
							<td class="center">
								<a href="<c:url value='/cms/info/popup/UpdatePopupManageView.do?menuTargetNo=${menuInfo.menuTargetNo}'/>&popupId=${item.popupId}" onClick="fn_pagedetail('<c:out value="${item.popupId}"/>','<c:out value="${item.popupId}"/>');return false;">
									<c:out value='${item.popupId}'/>
								</a>
							</td>
							<td class="center"><c:out value='${item.popupNm}'/></td>
							<td class="center"><c:out value='${item.ntceBgnde}'/>:<c:out value='${item.ntceBgndeTime}'/>~<c:out value='${item.ntceEndde}'/>:<c:out value='${item.ntceEnddeTime}'/></td>
							<td class="center"><c:out value='${item.ntceAt}'/></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
