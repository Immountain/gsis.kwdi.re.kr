...
<c:set var="pageTitle">이미지 게시판</c:set>
		...
		<div class="white-box">
			<div class="rows">
				...
				<span class="select-outline">
					 <select name="category" title="<spring:message code="title.searchCondition" />">
							<option selected value=''>선택하세요</option>

								<c:forEach items="${categoryList}" var="item" varStatus="status">
									<option value="${item.boardCategoryId}"  <c:if test="${searchVO.category == item.boardCategoryId}">selected="selected"</c:if> >${item.boardCategoryNm}</option>
								</c:forEach>
							</select>
				</span>
				...
				<tbody>
					...
					<c:forEach items="${list}" var="item" varStatus="status">
						<tr>
							...
							<td class="center"><c:out value="${item.categoryNm}"/></td>
							<td class="center">
