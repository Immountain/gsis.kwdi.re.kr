...
function insertMenuList() {
	...
	document.menuManageVO.action = "<c:url value='/cms/mnu/mpm/InfoMenuListInsert.do'/>";
	menuManageVO.submit();
...
function updateMenuList() {
	...
	document.menuManageVO.action = "<c:url value='/cms/mnu/mpm/InfoMenuListUpdt.do'/>";
	menuManageVO.submit();
}
...
function deleteMenuList() {
    if(!fn_validatorMenuList()){return;}
    if(document.menuManageVO.tmp_CheckVal.value != "U"){alert("<spring:message code="comSymMnuMpm.menuList.validate.checkVal" />"); return;} //상세조회시는 수정혹은 삭제만 가능합니다.
	document.menuManageVO.action = "<c:url value='/cms/mnu/mpm/InfoMenuListDelete.do'/>";
	menuManageVO.submit();
}
...
function selectMenuList() {
    document.menuManageVO.action = "<c:url value='/cms/mnu/mpm/InfoMenuListSelect.do'/>";
...
function selectMenuListTmp() {
	document.menuManageVO.req_RetrunPath.value = "/cms/mnu/mpm/InfoMenuList";
    document.menuManageVO.action = "<c:url value='/cms/mnu/mpm/InfoMenuListSelectTmp.do'/>";
    document.menuManageVO.submit();
}
    ...
    $(document).ready(function () {
        ...
        $('#popupProgrmFileNm').click(function (e) {
			...
			ax5modal.open({
				...
				iframe: {
					method: "get",
					url: '<c:url value='/sym/prm/InfoProgramListSearch.do'/>',
					param: {}
				}
			}, function (data) {
        ...
        $('#popupUpperMenuId').click(function (e) {
			...
			ax5modal.open({
				...
				iframe: {
					method: "get",
					url: '<c:url value='/cms/mnu/mpm/InfoMenuListSelectMvmnNew.do'/>',
					param: {}
				}
			}, function (d) {
			});
		...
		$('#auth').click(function (e) {
			...
			ax5modal.open({
				...
				iframe: {
					method: "get",
					url: '<c:url value='/cms/mnu/auth/view.do'/>',
					param: {
						cmsMemuId: $('input[name=menuNo]').val()
					}
				}
			}, function (d) {
			});
...
<div id="content">
	...
	<div class="subView">
		...
		<form name="menuManageVO" action ="<c:url value='/cms/mnu/mpm/InfoMenuListInsert.do' />" method="post">
			<input type="hidden" name="req_RetrunPath" value="/cms/mnu/mpm/InfoMenuList">
			...
			<div class="white-box toolbar">
				...
				<div class="rows">
					...
					<a class="btn" href="<c:url value='/cms/mnu/mpm/InfoMenuListSelect.do'/>" onclick="initlMenuList(); return false;" title="<spring:message code="button.init" />"><spring:message code="button.init" /></a>
				...
				<div class="rows">
					...
					<table class="wTable tree-table" >
						...
						<tr>
							...
							<td class="left">
								<input name="menuNo" type="text" value=""  maxlength="10" title="<spring:message code="comSymMnuMpm.menuList.menuNo" />" />
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="left">
								...
								<a id="auth" class="btn sub" href="/cms/mnu/mpm/InfoMenuListSelectMvmn.do" target="_blank" title="권한설정" style="selector-dummy:expression(this.hideFocus=false);">권한설정</a>
							</td>
						</tr>
						<tr>
							...
							<td class="left">
								...
								<a id="popupUpperMenuId" class="btn sub" href="/cms/mnu/mpm/InfoMenuListSelectMvmn.do" target="_blank" title="<spring:message code="comSymMnuMpm.menuList.upperMenuId" />" style="selector-dummy:expression(this.hideFocus=false);"><spring:message code="comSymMnuMpm.menuList.mvmnMenuList" /></a><!-- 메뉴선택 검색 -->
							</td>
						</tr>
						<tr>
							<th><spring:message code="comSymMnuMpm.menuList.progrmFileNm" /> <span class="pilsu">*</span></th><!-- 파일명 -->
							<td class="left">
								<input name="progrmFileNm" id="progrmFileNm" type="text" size="30" value=""  maxlength="60" title="<spring:message code="comSymMnuMpm.menuList.progrmFileNm" />" style="width:190px" readonly />
