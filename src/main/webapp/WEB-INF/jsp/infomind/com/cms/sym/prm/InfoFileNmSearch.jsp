...
function linkPage(pageNo){
	...
	document.progrmManageForm.action = "<c:url value='/sym/prm/InfoProgramListSearch.do'/>";
...
function fn_pagedetail(_progrmFileNm){

	var data ={
		progrmFileNm : _progrmFileNm,
	}
...
<body>
<form name="progrmManageForm" action ="<c:url value='/sym/prm/InfoProgramListSearch.do'/>" method="post">
	...
	<table class="board_list">
		...
		<tbody>
			<c:forEach var="result" items="${list_progrmmanage}" varStatus="status">
			  <tr>
				  <td class="center" style = "cursor:pointer;" onclick="fn_pagedetail('<c:out value="${result.progrmFileNm}"/>');"><c:out value="${(searchVO.pageIndex-1) * searchVO.pageSize + status.count}"/></td>
				  <td class="center" style = "cursor:pointer;" onclick="fn_pagedetail('<c:out value="${result.progrmFileNm}"/>');"><c:out value="${result.progrmFileNm}"/></td>
				  <td class="center" style = "cursor:pointer;" onclick="fn_pagedetail('<c:out value="${result.progrmFileNm}"/>');"><c:out value="${result.progrmKoreanNm}"/></td>
			  </tr>
			 </c:forEach>
		</tbody>
	</table>
