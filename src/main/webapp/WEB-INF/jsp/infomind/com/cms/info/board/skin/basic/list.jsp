...
<div class="sub subView">
	...
	<form name="searchVO" action="<c:url value='/cms/info/board/boardItemList.do'/>" method="post" onSubmit="fn_info_search_page(); return false;">
		<!-- 검색영역 -->
		<!-- 검색조건선택 -->
		<div class="white-box">
			<div class="rows">
...
목
				...
				<tbody>
					...
					<c:forEach items="${list}" var="item" varStatus="status">
						<tr>
							...
							<td class="center">
								<a href="<c:url value='/cms/info/board/boardItemView.do?menuTargetNo=${menuInfo.menuTargetNo}'/>&itemId=${item.itemId}" onClick="fn_detail('<c:out value="${item.itemId}"/>','<c:out value="${item.itemId}"/>');return false;">
									<c:out value="${item.title}"/>
								</a>
							</td>
							<td class="center"><c:out value="${item.memo}"/></td>
							<td class="center"><c:out value="${item.regId}"/></td>
		...
		<input name="boardId" id="boardId" type="hidden" value="${searchVO.boardId}">2222
