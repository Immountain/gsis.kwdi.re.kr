...
<div class="sub subView">
	...
	<h2 class="stitle">
		<i class='bx bxs-dashboard' ></i>베너 <spring:message code="title.list" />
	</h2>
		...
		<div class="white-box">
			<div class="rows">
				...
				<span class="select-outline">
					 <select name="bannerGroupId" title="<spring:message code="title.searchCondition" />">
							<option selected value=''>선택하세요</option><!-- 선택하세요 -->
							<c:forEach items="${bannerGroup}" var="item" varStatus="status">
								<option value="${item.bannerGroupId}"  <c:if test="${searchVO.bannerGroupId == item.bannerGroupId}">selected="selected"</c:if> >${item.bannerGroupNm}</option><!-- 페이지그룹 -->
							</c:forEach>
						</select>
				</span>
				...
				<button type="button" class="button main" onclick="location.href='<c:url value='/cms/info/banner/RegistBannerView.do?menuTargetNo=${menuInfo.menuTargetNo}' />'"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"  ><spring:message code="button.create" /></button>
			</div>
		</div>
