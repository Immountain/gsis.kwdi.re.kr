...
<script type="text/javascript">
    ...
    ******************************************************** */
	...
	function fnUpdt(){
		...
		var	API_SERVER = "<c:url value='/cms/wj/program/wjProgramInfoContentUpdate.do'/>";
		editorSave();
	...
	<div class="rows white-box">
		...
		<table class="landscape" summary="<spring:message code="common.summary.list" arguments="${pageTitle}" />">
				<td class="left" colspan="3">
					...
					<c:forEach var="result" items="${resultVO.listLang}" varStatus="status">
						<%--						<sctipt>--%>
						<%--							var codeEditor = $ifx.generateCodeEditor(resultVO.listLang, {--%>
						<%--							useThemeSelector: true--%>
						<%--							});--%>
						<%--						</sctipt>--%>
						...
						<table class="landscape" summary="<spring:message code="common.summary.list" arguments="${pageTitle}" />" style="display: none;" data-incms-id="${result.langCode}">
							<tr>
								...
								<td class="left">
									<form:input path="listLang[${status.index}].programNm" size="20" readonly="true"/>
								</td>
								<th width="10%"><label for="listLang[${status.index}].programPlace">프로그램장소<span class="pilsu">*</span></label></th>
								<td class="left">
									<form:input path="listLang[${status.index}].programPlace" size="20" maxlength="250" readonly="true"/>
								</td>
							</tr>
							<tr>
								<th width="10%"><label for="listLang[${status.index}].contentInfoDecode">내용<span class="pilsu">*</span></label></th>
								<td class="left" colspan="3">
									<textarea name="listLang[${status.index}].contentInfo">${result.contentInfoDecode}</textarea>
								</td>
							</tr>
						</table>
					</c:forEach>
