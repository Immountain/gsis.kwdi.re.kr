...
<script type="text/javascript">
        ...
        firstGrid.setConfig({
            ...
            columns: [
             //   {key: "dataYear", label: "년도", align: "center", width: 60},
                {
                    label: "2020년 이전", columns: [ // child columns
                        {key: "dataYear", label: "2020년 이후"}
                ...
                {
                    label: "야간에 혼자 걷기 두려운 곳이 있음", columns: [ // child columns
                        {key: "cdmData1", label: "야간에 혼자 걷기 불안함",  width: 150 ,formatter: "money",editor: {type: "text"}},
                ...
                {
                    label: "가로등이 없어서", columns: [ // child columns
                        {key: "cdmData2", label: "생활권 내에 보행안전 시설 부족",  width: 150 ,formatter: "money",editor: {type: "text"}},
                ...
                {
                    label: "인적이 드물어서", columns: [ // child columns
                        {key: "cdmData3", label: "생활권 내에 치안 시설 부족",  width: 150 ,formatter: "money",editor: {type: "text"}},
                ...
                {
                    label: "우범지역이므로", columns: [ // child columns
                        {key: "cdmData4", label: "생활권 내에 인적 드묾",  width: 150 ,formatter: "money",editor: {type: "text"}},


                    ]
                },
                {
                    label: "CCTV가 없어서", columns: [ // child columns
                        {key: "cdmData5", label: "생활권 내에 우범 지역 존재", width: 150 ,formatter: "money",editor: {type: "text"}},


                    ]
                },
                {
                    label: "기타", columns: [ // child columns
                        {key: "cdmData6", label: "신문, 뉴스 등에서 사건, 사고 자주 접해서",  width: 150 ,formatter: "money",editor: {type: "text"}},


                    ]
                },
                {
                    label: "두려운 곳이 없음", columns: [ // child columns
                        {key: "cdmData7", label: "기타",  width: 150 ,formatter: "money",editor: {type: "text"}},
    ...
    function gridExcelUplad() {
        ...
        $ifx.ajax('<c:url value="/cms/gsis/f02/upload.do"/>', {
            method: 'POST',
            processData: false,
            contentType: false,
            data: formData,
    ...
    function saveGrid() {
     ...
     if(!confirm('데이터을 생성 하시겠습니까?')) return false;
        ...
        $ifx.ajax('<c:url value='/cms/gsis/f02/updateObject.do' />', {
            method: "POST",
    ...
    function download() {
        ...
        form.setAttribute("action", "<c:url value="/cms/gsis/f02/download.do"/>"); //요청 보낼 주소
